    ...
    <div class="row">
        <div class="col-lg-8">
            <a href="/dashboard/productions" class="btn btn-success mb-3"> <span data-feather="arrow-left"></span>
                Kembali
                ke Semua Produksi</a>
            <a href="/dashboard/productions/{{ $production->id}}/edit" class="btn btn-warning mb-3"> <span
                    data-feather="edit"></span> Ubah</a>
            <form action="/dashboard/productions/{{ $production->id }}" method="post" class="d-inline">
                @method('delete')
                @csrf
                <button class="btn btn-danger mb-3"
                    onclick="return confirm('Apakah anda yakin ingin menghapus produksi ini?')"><span
